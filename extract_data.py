...
def extract_data(topics, filename, jsonfile, base_data_folder, args):
    ...
    mat3 = None
    mat4 = None
    mat4 = np.array([[ 4.89630558e-12,  4.89641661e-12,  1.00000000e+00, -7.97960000e-02],
 [-1.00000000e+00, -2.22044605e-16,  4.89641661e-12,  2.20000000e-02],
 [ 0.00000000e+00, -1.00000000e+00,  4.89630558e-12,  2.15172000e-01],
 [ 0.00000000e+00,  0.00000000e+00,  0.00000000e+00,  1.00000000e+00]])

    # since there is at least one bagfile with no tf_static
    palm_wrist_roll_translation = [0.012, 0.0, 0.1405]
    palm_wrist_roll_rotation = [0., 0., 1., -1.03411553555e-13]
    palm_wrist_roll = fromTranslationRotation(palm_wrist_roll_translation, palm_wrist_roll_rotation)
    ...
    for topic, msg, t in bag.read_messages(topics=topics_to_read):
        ...
        if (topic == topics['wrench']):
        ...
        elif (topic == topics['joint_states'] and mat1 is not None and mat2 is not None and mat3 is not None and mat4 is not None):
            positions = [t.to_sec()]
            velocities = [t.to_sec()]
            effort = [t.to_sec()]
            ...
            if joint_state_positions is None:
                ...
                joint_state_positions = np.vstack((joint_state_positions, positions[np.newaxis, :]))

            if joint_state_velocities is None:
                joint_state_velocities = velocities[np.newaxis, :]
            else:
                joint_state_velocities = np.vstack((joint_state_velocities, velocities[np.newaxis, :]))

            if joint_state_effort is None:
                joint_state_effort = effort[np.newaxis, :]
            else:
                joint_state_effort = np.vstack((joint_state_effort, effort[np.newaxis, :]))
            ...
            for transform in msg.transforms:
                if (transform.header.frame_id == 'base_link' and transform.child_frame_id == 'torso_lift_link'):
                    mat1 = get_transform_matrix(transform)
                    #print(mat1[2][3])
                elif (transform.header.frame_id == 'torso_lift_link' and transform.child_frame_id == 'head_pan_link'):
                    mat2 = get_transform_matrix(transform)
                elif (transform.header.frame_id == 'head_pan_link' and transform.child_frame_id == 'head_tilt_link'):
                    mat3 = get_transform_matrix(transform)
                elif (transform.header.frame_id == 'head_tilt_link' and transform.child_frame_id == 'head_rgbd_sensor_link'):
                    mat4 = get_transform_matrix(transform)
                elif (transform.header.frame_id == 'base_link' and transform.child_frame_id == 'arm_lift_link'):
                    arm_lift_base = get_transform_matrix(transform)
                elif (transform.header.frame_id == 'arm_lift_link' and transform.child_frame_id == 'arm_flex_link'):
                    arm_flex_lift = get_transform_matrix(transform)
                elif (transform.header.frame_id == 'arm_flex_link' and transform.child_frame_id == 'arm_roll_link'):
                    arm_roll_flex = get_transform_matrix(transform)
                elif (transform.header.frame_id == 'arm_roll_link' and transform.child_frame_id == 'wrist_flex_link'):
                    wrist_flex_arm_roll = get_transform_matrix(transform)
                elif (transform.header.frame_id == 'wrist_flex_link' and transform.child_frame_id == 'wrist_ft_sensor_mount_link'):
                    wrist_flex_to_sensor_mnt = get_transform_matrix(transform)
                elif (transform.header.frame_id == 'wrist_ft_sensor_mount_link' and transform.child_frame_id == 'wrist_ft_sensor_frame'):
                    wrist_sensor_frame = get_transform_matrix(transform)
                elif (transform.header.frame_id == 'wrist_ft_sensor_frame' and transform.child_frame_id == 'wrist_roll_link'):
                    wrist_sensor_frame_to_roll = get_transform_matrix(transform)
                elif (transform.header.frame_id == 'wrist_roll_link' and transform.child_frame_id == 'hand_palm_link'):
                    palm_wrist_roll = get_transform_matrix(transform)


        if all(var is not None for var in [mat1, mat2, mat3, mat4, arm_lift_base, arm_flex_lift, arm_roll_flex, wrist_flex_arm_roll, wrist_flex_to_sensor_mnt, wrist_sensor_frame, wrist_sensor_frame_to_roll]):
            base_to_camera = mat1.dot(mat2).dot(mat3).dot(mat4)
            base_to_palm = arm_lift_base.dot(arm_flex_lift).dot(arm_roll_flex).dot(wrist_flex_arm_roll).dot(wrist_flex_to_sensor_mnt).dot(wrist_sensor_frame).dot(wrist_sensor_frame_to_roll).dot(palm_wrist_roll)
            ...
            if palm_position is None:
                ...
                palm_position = np.vstack((palm_position, camera_to_palm[np.newaxis, :]))
    ...
    filename = os.path.join(data_folder, 'palm_positions')
...
def resample(data_folder, args):
    ...
    original_files = ['joint_state_efforts.npy',
                      'joint_state_positions.npy',
                      'joint_state_velocities.npy',
                      'wrench.npy',
                      'camera_matrix.npy',
                      'palm_positions.npy']
    resampled_files = ['joint_state_efforts_resampled.npy',
                      'joint_state_positions_resampled.npy',
                      'joint_state_velocities_resampled.npy',
                      'wrench_resampled.npy',
                      'camera_matrix_resampled.npy',
                      'palm_positions_resampled.npy']
    ...
    for tt, tt2 in zip(original_files, resampled_files):
        ...
        if 'joint_state_positions' in tt:
...
def main():
    ...
    parser.add_argument('-b', '--include_base', action="store_true", help="generate base velocities")
    parser.add_argument('-s', '--single_bagfile', type=str, default=None, help="specify filename of single bagfile in bagfile_path")
    parser.add_argument('-j', '--single_jsonfile', type=str, default=None, help="specify filename of single json file in bagfile_path")
    args = parser.parse_args()

    path = args.bagfile_path
    base_data_folder = args.base_data_folder
    topics = {'head': '/hsrb/head_rgbd_sensor/rgb/image_raw',
